...
class ToolCache:
  ...
  def _build_cache_key(
    ...
    if self._config.include_app_name:
      parts.append(f"app:{tool_context.invocation_context.app_name}")
    if self._config.include_user_id:
      parts.append(f"user:{tool_context.invocation_context.user_id}")
    if self._config.include_session_id:
      parts.append(f"session:{tool_context.invocation_context.session_id}")

    # Include tool name and serialized args
  ...
  def _get_session_key(self, tool_context: ToolContext) -> str:
    ...
    return f"{ctx.app_name}:{ctx.user_id}:{ctx.session_id}"
